...
export default function ItemsDisplay ({ title, items, seeAll, setSeeAll, specialSales }: props) {
    ...
    const getTotalReview = (obj: Record<string, Review>) => {
    ...
    const reviewList = Object.values(product.reviews);
                    ...
                    <TouchableOpacity onPress={handleItemPress}>
                        <View style={styles.card}>
                            <View style={{width: '100%', position: 'relative'}}>
                                <Image source={item.media[0]} style={{width: '100%', height: 250}} />
                            ...
                            {specialSales && item.discount !== undefined ?  (
                                <View>
                                    <Text style={{fontSize: 18, fontWeight: 500}}><IconSymbol lib="Font6" name="naira-sign" size={13} />{discountPrice({price: item.price, discount: item.discount})}</Text>
                                    <Text style={{color: 'grey', textDecorationLine: 'line-through'}}><IconSymbol lib="Font6" name="naira-sign" size={13} />{item.price}</Text>
                                </View>
                            ) : (
                            ...
                            <View style={{display: 'flex', flexDirection: 'row', gap: 10}}>
                                ...
                                <Text>{getTotalReview(item.reviews)} Reviews</Text>
                            </View>
                        </View>
                    </TouchableOpacity>
                )}
